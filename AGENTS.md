...
## Testing Guidelines
Vitest drives unit and integration coverage. Name files `*.test.ts`, keep arrange/act/assert sections clear, and use `bun run test:coverage` for the calibrated 80% coverage gate. Use `bun run test:run`, `bun run test:modules`, `bun run test:integration`, and `bun run test:ui-dom` for focused Vitest work; use `bun run vitest:ui` only as a developer UI launcher, not as verification. Prefer `jsdom + React Testing Library + jest-dom + user-event` for new component-level UI tests instead of string-based markup assertions. Use `bun run test` before handoff so the Bun smoke layers run too. For end-to-end work, follow `docs/E2E_TESTING_GUIDE.md`: use `bun run verify:e2e-smoke` for the required hermetic browser path, use HTTP checks first where they are sufficient, and start `bun run dev` only for optional manual QA or local investigation.
