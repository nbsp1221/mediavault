...
export function createServerPlaybackServices(
  ...
  const lazyTokenService: PlaybackTokenServicePort = {
    issue: input => getTokenService().issue(input),
    validate: token => getTokenService().validate(token),
  };
