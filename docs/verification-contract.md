...
- `test:mutation:changed` is the local changed-file mutation gate. It discovers staged, unstaged, and untracked local production files relative to `HEAD`, filters them through the calibrated changed-file production scope, and invokes StrykerJS with the root `stryker.config.mjs` plus `--mutate <comma-separated-files>`. It does not use Stryker incremental mode because the local Git change set is this gate's source of truth and the report must describe the current changed-file scope only. When no eligible changed production files remain after filtering, it exits successfully without running Stryker and prints `No changed production files require mutation validation.` It inherits the shared `thresholds.break: 70` mutation-score floor.
